...
<aside class="w-64 bg-slate-900/80 backdrop-blur-xl border-r border-slate-800/50 flex flex-col relative overflow-hidden">
    ...
    <nav class="flex-1 p-4 space-y-1">
        <a href="trade/holdings" class="nav-item ${param.activePage == 'holdings' ? 'active' : ''} flex items-center space-x-3 px-4 py-3 ${param.activePage == 'holdings' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium' : 'text-slate-400 hover:text-slate-100 hover:bg-slate-800/50'} rounded-xl transition-all duration-300">
    ...
    <div class="p-4 border-t border-slate-800/50">
        <div class="flex items-center space-x-3 px-4 py-3 hover:bg-slate-800/50 rounded-xl cursor-pointer transition-all duration-300 group">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-slate-100 truncate"><%=name %></p>
